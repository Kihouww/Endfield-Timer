...
# **⚠️注意 CAUTION⚠️**
>1. 使用前，需要关闭windows内置的诸如 **护眼模式、HDR** 等任何会使游戏画面默认颜色偏色的选项！！！
>2. 请保证游戏在 **全屏状态** 下运行！
>3. 多显示器的玩家，请将游戏运行显示器 **设置为主显示器** 后，再启动该程序。

1.  **启动程序**：双击目录下的 `bosstimer.exe`。
2.  **选择目标**：点击插件左上角，选择当前要挑战的 BOSS。
3.  **自动运行**：保持程序运行，程序将在开战前自动进入 READY 状态。
4.  **重置状态**：单击 **“重置计时”**，或 **切换boss**，可重置计时至待命状态。
