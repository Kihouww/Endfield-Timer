...
* **底层为图像识别**
    不读取内存，不注入游戏。完全基于**画面特定 UI 像素的状态变化**作为判断战斗起止的核心依据，判定轻量、精准。
...
| **终止** | 识别到 **“挑战成功”** 或结算字样弹出 | **停止计时**<br>*(内置回溯，自动扣除 UI 弹出延迟的 0.22 秒)* |
